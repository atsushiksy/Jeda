...
1. Lexical Elements
  ...
  1.4 Keywords
    	...
    	abstract	advice	array	aspect	assoc	bit
    	bothedge	break	breakpoint	call	case	casex
    	casez	class	class_type	clone	connectionset	continue
    	default	depth	do	double	drive	else
    	enum	export	extends	extern	false	final
    	float	for	forever	fork	global	if
    	initial	inout	input	int	join	knife
    	list	local	long	marray	negedge	new
    	node	null	output	overwrite	p_and	p_or
    	port	portset	posedge	protected	repeat	return
    	sample	signal	skew	spoon	static	string
    	stridx	strong	super	teminate	terminate	this
    	true	typedef	var	verilog	void	weak
    	while
