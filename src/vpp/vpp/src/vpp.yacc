...
%%
source_text : /* empty */
	| if_declaration source_text
	| ifdef_declaration source_text
	| ifndef_declaration source_text
	| else_declaration source_text
        | endif_declaration source_text
	| for_declaration source_text
	| endfor_declaration source_text
	| while_declaration source_text
	| endwhile_declaration source_text
	| switch_declaration source_text
	| case_declaration source_text
	| breaksw_declaration source_text
	| default_declaration source_text
	| endswitch_declaration source_text
	| let_declaration source_text
	| other_stuff source_text
	| error {
	    set_to_last_mode();
	    yyclearin;
	    yyerrok;
...
for_declaration : TTFOR {set_to_vpp_mode();}
		   '(' identifier '=' expression {vp1 = create_variable_ref($4,$6,TRUE);}
...
let_declaration : TTLET {set_to_vpp_mode();} identifier '=' expression optional_semi vpp_line_termination_opt
		{
		  if (loop_struct_count > 0) {
		    if (code_pointer != NULL)
		      cstack = push(cstack, create_generic_ll(CODE_POINTER,NULL,
...
identifier : TTNAME {$$ = (char *) strdup(ttid);}
	;
