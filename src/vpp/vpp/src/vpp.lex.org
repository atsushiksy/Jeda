...
<INITIAL,MPP>"/*"  {eat_up_comment(FALSE);}
...
<INITIAL>`autoexpand_vectomets	{printf("%s",yytext);}
...
<INITIAL>`{NAME}  {print_variable(yytext+1, FALSE);}
<COPY>`{NAME}  {store_it(yytext,TRUE,FALSE);}
