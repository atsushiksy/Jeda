/*
 * regcomp and regexec -- regsub and JD_regerror are elsewhere
...
#define	ANYOF	 9	/* sv   Match character in this class. */
...
#define	FAIL(m)	{ JD_regerror(m); return(NULL); }
...
static char *regxend ;  /* end of the reg expression */
...
/**********************************************************************
 * some functions from Perl5 util.c
 **********************************************************************/
char
...
char *exp;
{
	...
	/* Dynamic allocation of startp, endp array. */
        r->nparens = regnpar - 1 ;
        r->startp = (char **)calloc( regnpar, sizeof(char*) ) ;
	if (r->startp == NULL)
		FAIL("out of space");
        r->endp = (char **)calloc( regnpar, sizeof(char*) ) ;
	if (r->endp == NULL)
		FAIL("out of space");
	...
	if (OP(regnext(scan)) == END) {		/* Only one top-level choice. */
		...
		if (flags&SPSTART) {
			...
			for (; scan != NULL; scan = regnext(scan))
				if (OP(scan) == EXACT && strlen(OPERAND(scan)) >= len) {
...
int *flagp;
{
	...
	*flagp |= flags&SPSTART;
	...
	if( paren )  ender = reganode(CLOSE,parno) ;
	else ender = regnode(END) ;
	/*
	...
	case '\\':
            ...
            case '5': case '6': case '7': case '8': case '9':
                {
					...
					goto defchar;
                        ...
                        *flagp |= HASWIDTH;
                        while (isDIGIT(*regparse)) regparse++;
                    }
                }
                break;
            case '\0':
	  ...
	  break;            
        default: {
            ...
            defchar:
            ret = regnode(EXACT);
            for (len = 0, p = regparse - 1; p <= regxend; len++)
            {
                oldp = p;
...
static char *regeol;            /* End of input, for $ check and such */
...
register char *string;
{
	...
	if (prog == NULL || string == NULL) {
		JD_regerror("NULL parameter");
		return(0);
	}

	/* Check validity of program. */
	if (UCHARAT(prog->program) != MAGIC) {
		JD_regerror("corrupted program");
		return(0);
	}
...
char *prog;
{
  ...
  while (scan != NULL) {
    ...
    switch (OP(scan)) {
      ...
      case ALNUM:
        if (!*reginput) return(0);
        if (!isALNUM(*reginput)) return(0);
        reginput++;
        break;
      case NALNUM:
        if (!*reginput) return(0);
      ...
      case REF:
        {
          ...
          s = regstartp[n];
          if (!s) return(0);
          if (!regendp[n]) return(0);
          if (s == regendp[n])
            break;
          /* Inline the first character, for speed. */
	  if (*s != *reginput ) return(0) ;
	  len = regendp[n] - s;
	  if (reginput + len > regeol) return(0);
	  if (len > 1 && strncmp(reginput, s, len) ) return(0) ;
	  reginput += len;
	}
	break;

      default:
        JD_regerror("memory corruption");
        return(0);
        break;
    }
   ...
   */
  JD_regerror("corrupted pointers");
  return(0);
}
...
char *p;
{
	...
	case ANYBUT:
		...
		break;
	case ALNUM:
		while (scan <= regeol && isALNUM(*scan)) {
			count++;
			scan++;
		}
		break;
	case NALNUM:
		...
		break;
	case SPACE:
		while (scan <= regeol && isSPACE(*scan)) {
			count++;
			scan++;
		}
		break;
	case NSPACE:
		...
		break;
	case DIGIT:
		while (scan <= regeol && isDIGIT(*scan)) {
			count++;
			scan++;
		}
		break;
...
regexp *r;
{
	...
	while (op != END) {	/* While that wasn't END last time... */
		...
		s += 3;
		if (op == ANYOF || op == ANYBUT || op == EXACT) {
...
char *op;
{
  ...
  switch (OP(op)) {
    ...
    case REFFL:
        ...
        break;
    case OPEN:
        sprintf(buf+strlen(buf), "OPEN%d", ARG1(op));
        p = NULL;
        break;
    case CLOSE:
        sprintf(buf+strlen(buf), "CLOSE%d", ARG1(op));
        p = NULL;
        break;
    case STAR:
        p = "STAR";
...
/* function to deallocate regexp structure */
void JD_regdealloc(r)
regexp *r ;
{
  if( r ) {
    if( r->startp ) free(r->startp) ;
    if( r->endp ) free(r->endp) ;
    free(r) ;
  }
}
