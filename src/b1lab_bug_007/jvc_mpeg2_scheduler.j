...
void gen_seq_eq_loop() {
   static int gen_seq_num=0;   //gen the number of the function
   int state;
   string trace_msg;
...
//       in_stub.ev_qe_send_ok.sync();
         ...
         } while (state==0); //if current data is empty,gen the next datda.


         if (gen_seq_num==10){          //when the mb data has send over
//            if (ig.mb_counter<u_mb_num[][]) {
            //printf("******************************u_mb_num=%d\n",u_mb_num);

            if (ig.mb_counter<2) {   
               gen_seq_num=9;       //send the left mb data
            }
            else {
               ig.mb_counter=0;     //reset the mb counter
             //<<  if (ig.slice_counter<u_slice_num[][]) {
               //printf("******************************u_slice_num=%d\n",u_slice_num);
               if (ig.slice_counter<2) {   
                  gen_seq_num=8;     //send the left slice data
               }
               else {
                  in_stub.is_pake_add_0=d_is_pake_add_0;   //return to default style
                  ig.slice_counter=0;   //reset the slice counter
                  if (ig.pic_counter<picture_num){   //send the left slice data
...
void copy_seq(){    //copy the default instant
   u_video=new d_video;
/*
