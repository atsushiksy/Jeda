...
/*
   ...
   u_get_vlc_table(Table_B_1,u_mb_addr_increment_index[n][p][q]);
   s="macroblock_address_increment_vlc";                  
   pack_qe(u_vlc_len,u_vlc_value,s); 
   
   macroblock_mode(m,n,p,q);
    
   if (u_macroblock_quant) {
      s="quantiser_scale_code_mb";                 
      pack_qe(5,u_quantiser_scale_code_mb[n][p][q],s);
   }
   if ((u_macroblock_motion_forward)||(u_macroblock_intra&&u_concealment_motion_vectors)) {                          
      motion_vectors(0);        
   }                                                  
   if (u_macroblock_motion_backward) {
      motion_vectors(1);
   }
   if (u_macroblock_intra&&u_concealment_motion_vectors) {
      s="market_bit";           
      pack_qe(1,u_market_bit,s);
   }
...
/*
   void macroblock_mode(int m,int n,int p,int q) {
         ...
         if (u_scalable_mode==2'b01) {  //spatial_scal 
            if (u_pic_spat_scal_ext_load) {
               case(u_picture_coding_type[m][n]) {                       
                  I: u_get_vlc_table(Table_B_5,u_macroblock_type_index); 
                  P: u_get_vlc_table(Table_B_6,u_macroblock_type_index); 
                  B: u_get_vlc_table(Table_B_7,u_macroblock_type_index); 
               }                                                                        
            } 
            else {
               case(u_picture_coding_type[m][n]) {                      
                  I: u_get_vlc_table(Table_B_2,u_macroblock_type_index);
                  P: u_get_vlc_table(Table_B_3,u_macroblock_type_index);
                  B: u_get_vlc_table(Table_B_4,u_macroblock_type_index);
               }                                                                       
            } // (u_pic_spat_scal_ext_load)                                               
         }
         else {
            if (u_scalable_mode==2'b10) {    //SNR_scal                                               
               u_get_vlc_table(Table_B_8,u_macroblock_type_index);                                                
            } 
            else {
               case(u_picture_coding_type[m][n]) {                       
                  I: u_get_vlc_table(Table_B_2,u_macroblock_type_index); 
      ...
      pack_qe(u_vlc_len,u_vlc_value,s);                                                            
      if ((u_spatial_temporal_weight_code_flag)&&(u_spatial_temporal_weight_code_inedx_0!=0)) {     
   ...
   void motion_vectors(int s) {
      if (u_motion_vector_counter) {
         if ((u_my_format==0)&&(u_dmv!=1)) {
            s="motion_vertical_field_select_0s";                    
         ...
         pack_qe(1,u_motion_vertical_field_select_0s[n][p][q],s);
         motion_vector(0,s);  
         s="motion_vertical_field_select_1s";                    
...
void motion_vector(int r,int s) {
         ...
         if (s==0) f_code=u_f_code00[m][n];
         else f_code=u_f_code10[m][n];
         if ((f_code!=1)&&(u_vlc_value!=0)) {
            l=u_get_motion_residual_len(m,n,s,0);
            s="motion_residual";                 
            pack_qe(l,u_motion_residual[n][p][q],s); 
         }
         if (u_dmv) {
