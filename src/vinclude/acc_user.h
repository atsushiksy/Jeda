...
XXTERN bool    acc_append_delays PROTO_PARAMS((handle object, ...));
XXTERN bool    acc_append_pulsere PROTO_PARAMS((handle object, double val1r, double val1x, ...));
XXTERN void    acc_close PROTO_PARAMS((void));
XXTERN handle *acc_collect PROTO_PARAMS((handle (*p_next_routine)(), handle scope_object, int *aof_count));
XXTERN bool    acc_compare_handles PROTO_PARAMS((handle h1, handle h2));
XXTERN bool    acc_configure PROTO_PARAMS((int item, char *value));
XXTERN int     acc_count PROTO_PARAMS((handle (*next_func)(), handle object_handle));
XXTERN int     acc_fetch_argc PROTO_PARAMS((void));
XXTERN char  **acc_fetch_argv PROTO_PARAMS((void));
XXTERN double  acc_fetch_attribute PROTO_PARAMS((handle object, ...));
XXTERN int     acc_fetch_attribute_int PROTO_PARAMS((handle object, ...));
XXTERN char   *acc_fetch_attribute_str PROTO_PARAMS((handle object, ...));
XXTERN char   *acc_fetch_defname PROTO_PARAMS((handle object_handle));
XXTERN int     acc_fetch_delay_mode PROTO_PARAMS((handle object_p));
XXTERN bool    acc_fetch_delays PROTO_PARAMS((handle object, ...));
XXTERN int     acc_fetch_direction PROTO_PARAMS((handle object_handle));
XXTERN int     acc_fetch_edge PROTO_PARAMS((handle acc_obj));
XXTERN char   *acc_fetch_fullname PROTO_PARAMS((handle object_handle));
XXTERN int     acc_fetch_fulltype PROTO_PARAMS((handle object_h));
XXTERN int     acc_fetch_index PROTO_PARAMS((handle object_handle));
XXTERN double  acc_fetch_itfarg PROTO_PARAMS((int n, handle tfinst));
XXTERN int     acc_fetch_itfarg_int PROTO_PARAMS((int n, handle tfinst));
XXTERN char   *acc_fetch_itfarg_str PROTO_PARAMS((int n, handle tfinst));
XXTERN int     acc_fetch_location PROTO_PARAMS((p_location location_p, handle object));
XXTERN char   *acc_fetch_name PROTO_PARAMS((handle object_handle));
XXTERN int     acc_fetch_paramtype PROTO_PARAMS((handle param_p));
XXTERN double  acc_fetch_paramval PROTO_PARAMS((handle param));
XXTERN int     acc_fetch_polarity PROTO_PARAMS((handle path));
XXTERN int     acc_fetch_precision PROTO_PARAMS((void));
XXTERN bool    acc_fetch_pulsere PROTO_PARAMS((handle path_p, double *val1r, double *val1e, ...));
XXTERN int     acc_fetch_range PROTO_PARAMS((handle node, int *msb, int *lsb));
XXTERN int     acc_fetch_size PROTO_PARAMS((handle obj_h));
XXTERN double  acc_fetch_tfarg PROTO_PARAMS((int n));
XXTERN int     acc_fetch_tfarg_int PROTO_PARAMS((int n));
XXTERN char   *acc_fetch_tfarg_str PROTO_PARAMS((int n));
XXTERN void    acc_fetch_timescale_info PROTO_PARAMS((handle obj, p_timescale_info aof_timescale_info));
XXTERN int     acc_fetch_type PROTO_PARAMS((handle object_handle));
XXTERN char   *acc_fetch_type_str PROTO_PARAMS((int type));
XXTERN char   *acc_fetch_value PROTO_PARAMS((handle object_handle, char *format_str, p_acc_value acc_value_p));
XXTERN void    acc_free PROTO_PARAMS((handle *array_ptr));
XXTERN handle  acc_handle_by_name PROTO_PARAMS((char *inst_name, handle scope_p));
...
XXTERN handle  acc_handle_itfarg PROTO_PARAMS((int n, void *suena_inst));
XXTERN handle  acc_handle_loconn PROTO_PARAMS((handle port_ref));
XXTERN handle  acc_handle_modpath PROTO_PARAMS((handle mod_p, char *pathin_name, char *pathout_name, ...));
XXTERN handle  acc_handle_notifier PROTO_PARAMS((handle tchk));
XXTERN handle  acc_handle_object PROTO_PARAMS((char *inst_name, ...));
...
XXTERN handle  acc_handle_port PROTO_PARAMS((handle mod_handle, int port_num, ...));
...
XXTERN handle  acc_handle_tchk PROTO_PARAMS((handle mod_p, int tchk_type, char *arg1_conn_name, int arg1_edgetype, ...));
XXTERN handle  acc_handle_tchkarg1 PROTO_PARAMS((handle tchk));
XXTERN handle  acc_handle_tchkarg2 PROTO_PARAMS((handle tchk));
XXTERN handle  acc_handle_terminal PROTO_PARAMS((handle gate_handle, int terminal_index));
XXTERN handle  acc_handle_tfarg PROTO_PARAMS((int n));
XXTERN handle  acc_handle_tfinst PROTO_PARAMS((void));
XXTERN bool    acc_initialize PROTO_PARAMS((void));
XXTERN handle  acc_next PROTO_PARAMS((int *type_list, handle h_scope, handle h_object));
...
XXTERN bool    acc_object_of_type PROTO_PARAMS((handle object, int type));
XXTERN bool    acc_object_in_typelist PROTO_PARAMS((handle object, int *type_list));
XXTERN int     acc_product_type PROTO_PARAMS((void));
XXTERN char   *acc_product_version PROTO_PARAMS((void));
XXTERN int     acc_release_object PROTO_PARAMS((handle obj));
XXTERN bool    acc_replace_delays PROTO_PARAMS((handle object, ...));
XXTERN bool    acc_replace_pulsere PROTO_PARAMS((handle object, double val1r, double val1x, ...));
XXTERN void    acc_reset_buffer PROTO_PARAMS((void));
XXTERN bool    acc_set_interactive_scope PROTO_PARAMS((handle scope, int callback_flag));
XXTERN bool    acc_set_pulsere PROTO_PARAMS((handle path_p, double val1r, double val1e));
XXTERN char   *acc_set_scope PROTO_PARAMS((handle object, ...));
XXTERN int     acc_set_value PROTO_PARAMS((handle obj, p_setval_value setval_p, p_setval_delay delay_p));
XXTERN void    acc_vcl_add PROTO_PARAMS((handle object_p, int (*consumer)(), char *user_data, int vcl_flags));
XXTERN void    acc_vcl_delete PROTO_PARAMS((handle object_p, int (*consumer)(), char *user_data, int vcl_flags));
XXTERN char   *acc_version PROTO_PARAMS((void));

/*---------------------------------------------------------------------------*/
/*----------------------- global variable definitions -----------------------*/
/*---------------------------------------------------------------------------*/

extern DLLISPEC bool acc_error_flag;
